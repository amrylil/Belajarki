...
<div class="min-h-screen bg-slate-50 font-[Poppins] flex flex-col pt-20">

    <div class="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between sticky top-20 z-30">
        <div class="flex items-center gap-4">
            <a href="{{ route('home') }}" class="p-2 hover:bg-slate-100 rounded-full transition text-slate-500">
    ...
    <div class="flex-1 max-w-7xl mx-auto w-full p-6 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white p-8 rounded-2xl border border-slate-100 mt-6 prose prose-slate max-w-none">
                @if($currentLesson->content)
                    {!! $currentLesson->content !!}
                @else
                    <p class="text-slate-400 italic">Tidak ada deskripsi teks untuk materi ini.</p>
                @endif
            </div>
        </div>
                ...
                <div class="max-h-[600px] overflow-y-auto">
                    @foreach($course->modules as $module)
                        <div x-data="{ open: true }">
                            <button @click="open = !open" class="w-full flex items-center justify-between p-4 bg-slate-50/80 hover:bg-slate-100 transition text-left border-b border-slate-100">
                            ...
                            <div x-show="open" class="bg-white">
                                @foreach($module->lessons as $lesson)
                                    ...
                                    <button wire:click="goToLesson('{{ $lesson->id }}')"
                                            class="w-full flex items-start gap-3 p-4 border-b border-slate-50 transition text-left {{ $isActive ? 'bg-indigo-50 border-indigo-100' : 'hover:bg-slate-50' }}">
                                        ...
                                        <div class="mt-0.5">
                                            ...
                                            @else
                                                @if($lesson->type == 'video')
                                                     <div class="w-5 h-5 rounded-full border-2 border-slate-200 flex items-center justify-center"><svg class="w-3 h-3 text-slate-300" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg></div>
                                                @else
                                                     <div class="w-5 h-5 rounded-full border-2 border-slate-200 flex items-center justify-center"><svg class="w-3 h-3 text-slate-300" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg></div>
                                                @endif
                                            @endif
                                        </div>
